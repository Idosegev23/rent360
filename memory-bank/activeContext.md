...
- **חדש**: מערכת לידים מוואטסאפ - טבלת `whatsapp_leads`, API לקליטת JSON מבוט, מנוע התאמה מתקדם, ו-UI לניהול לידים מוואטסאפ.
...
## צעדים הבאים
- הוספת Webhook לקליטת לידים (Supabase Edge Function `leads-ingest`) עם אימות Bearer, נרמול טלפון, פרסינג `price_range` ומיפוי העדפות, ו‑upsert ל‑`leads`.
